...
**方法**: `git credential fill`（stdio 输入）提取 token → Node.js REST API。API 返回 HTTP 204，x-ratelimit-remaining=4999。注：本次 `git-credential-manager.exe get` pipe 方式失效，改用 `git credential fill` 成功。
