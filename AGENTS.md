...
- `scripts/`:注入脚本(**Hexo 自动加载 `scripts/` 目录下所有 js 作为插件**,无需注册;`_config.yml` 里曾经的 `scripts:` 列表是无效配置键,2026-08-07 已删除)。**旧 v1 日报生成器已移至 `_reference/football-daily-v1.js`,勿移回 `scripts/`——放回即会在每次 `hexo generate` 时自动执行,覆盖 v2 日报**
  - `hero-inject.js`:首页 hero(ASCII 河流 shader + 左下角 typed 副标题 + hero 下方 `.hero-fade` 渐变带 v7)——渐变带 = 动画自身的延伸,不是手调色带:river-hero.js 在主循环 `gpuTimedDraw()` 后与 `freezeStaticHero()` 快照后各回调一次 `window.__heroFrameHook(canvas)`(同一同步任务,WebGL 缓冲未被合成清除,外部 rAF/定时器取样会得到纯黑帧,勿绕行);heroJs 以 15fps 取 canvas 底边 mh 行(按显示比例换算 ≈ 底边以上 27% 场景)**垂直翻转**铺满渐变带(带顶 = 动画底边那一行,星野/山体纹理无缝续入——薄条拉伸会产生"点变线"分界,必须用整块镜像),再由「S 曲线渐晕(顶 8% 不擦、88% 擦净,勿改回陡擦——急擦会顶出"消失线") + 粗颗粒径向团块 + 细噪声」destination-out 遮罩溶解成碎尾(遮罩仅 resize 时重建);色彩衬底 = 底边**暗部 45 分位**采样色(勿用均值——均值偏亮,半擦除区会透出"贴膜"雾感)→ 平坦 14% → 动画夜色系(#101E38/#1B3566/#46608E/#8BA7C6)→ 2 个中间色拉长融入 `--page-bg`(监听 window 的 `themechange`),**关键帧路径必须经 `samplePath()` smoothstep 重采样为 48 级 stop**——导数逐帧归零,直接 addColorStop 关键帧会在每个 stop 处留导数突变("结节"),尾段 89%→100% 急变白就是 2026-08-07 前那道可见分界线的成因;全带另覆一层静态白色细颗粒(grainCv,alpha 随深度衰减 70% 以下归零)防 banding、统一镜像碎尾与平坦衬底质感;带高 clamp(240px,32vh,420px);镜像以 0.95 不透明度合成到 placeholder 底色 #15397A 上(hero canvas 本身只有 0.95 不透明,底下垫 placeholder,不补偿会在交界处露色阶),挂钩沉默 >800ms 且静态兜底图在播时改从 `#heroStill` img 取样;另叠加稀疏同款 RAMP_ASCII 字符 12fps 闪烁(密度限于顶部 55%,颜色取底边亮粒子);IntersectionObserver 不可见即停。**羽化兜底**:hero canvas/still/placeholder 底部 22% 带 `mask-image` 渐隐,露出 `section.hero` 底色 #0B1220(= 渐变带顶部兜底色);首次取样成功才给 body 加 `hero-fade-live` 摘除遮罩——取样链在用户会话里断掉时(扩展注入/驱动丢缓冲/死缓存)带顶退回默认深色,亮场景底边会顶出直线分界,羽化保证任何失败路径只剩软过渡(2026-07-23 用户现场出现但脚本复现不了的硬边,据此加固);**section.hero 底色必须从 #0E2F7E 保持为 #0B1220**,否则羽化终点与带顶不同色会重新露出阶跃
